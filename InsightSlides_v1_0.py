...
class LicenseManager:
            ...
            try:
                with open(LICENSE_FILE, 'r', encoding='utf-8') as f:
                    ...
                    if data.get('key'):
                        ...
                        is_valid, _ = self._validate_key_full(data['key'])
                        if is_valid:
                            return data
                        else:
    ...
    def _validate_key_full(self, key: str) -> Tuple[bool, str]:
        """Validate key and check expiration. Returns (is_valid, license_type)"""
        if not key:
            return False, 'free'
        ...
        # New format: TRIAL-XXXXXX-YYYYMMDD (trial)
        ...
        # New format: PRO-XXXX-XXXX-XXXX (permanent) or PRO-XXXX-XXXX-2025 (annual)
        ...
        if len(parts) == 5:
            ...
            if parts[4] == self._generate_checksum(key_body):
                license_type = 'professional' if parts[0] == 'PRO' else ('standard' if parts[0] == 'STD' else 'free')
                return True, license_type
    ...
    def _validate_key(self, key: str) -> bool:
        is_valid, _ = self._validate_key_full(key)
        return is_valid
    
    def _get_license_type_from_key(self, key: str) -> str:
        _, license_type = self._validate_key_full(key)
        return license_type
    ...
    def generate_license_key(self, user_id: str, license_type: str = 'standard', days_valid: int = 365) -> str:
        ...
        return f"{key_body}-{self._generate_checksum(key_body)}"
    
    def activate_license(self, key: str) -> Tuple[bool, str]:
        if not key:
            return False, t('license_enter_prompt')
        
        is_valid, license_type = self._validate_key_full(key)
        if not is_valid:
        ...
        if parts[0] == 'TRIAL' and len(parts) == 3:
            expires = f"{parts[2][:4]}-{parts[2][4:6]}-{parts[2][6:8]}"
        elif len(parts) == 4 and parts[3].isdigit() and len(parts[3]) == 4:
            expires = f"{parts[3]}-12-31"
